  ...
  <div class="articles">
    ...
    <div class="article-list">
      <transition name="module" mode="out-in">
        <transition-group name="fade" tag="div" v-if="article.data.list && article.data.list.length">
          <list-item :key="index"
                     :article="item"
                     @click.native="toDetail(item)"
                     v-for="(item, index) in article.data.list"></list-item>
        </transition-group>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      canLoadMore() {
        const { currentPage, totalPage } = this.article.data.pagination
        const hasArticles = this.article.data.pagination
        return hasArticles ? currentPage < totalPage : false
      }
    },
    methods: {
      toDetail(article) {
